...
public class ExtractFullCorpus {
		...
		for (int patID : patientProcedureDesc.keySet()) {
			...
			if (!skp) {
				...
				for (int i = 0; i < reportList.size(); ++i) {
					++counter;
					PrintWriter pw = new PrintWriter(outputFolder+ "fullCorpus/reports/" + patID + "_" + i + ".txt", "UTF-8");
					pw.println(reportList.get(i).toLowerCase());
					pw.close();
